...
namespace ClubMembershipApplication.FieldValidators
{
    public class UserRegistrationValidator:IFieldValidator
    {
        ...
        CompareFieldsValidDel _comparisonFieldsValidDel = null;
        ...
        public void InitializeValidatorDelegates()
        {
            ...
            _comparisonFieldsValidDel = CommonFieldValidatorFunctions.FieldsCompareValidDel;
        }
        private bool ValidField(int fieldIndex,string fieldValue, string[] fieldArray,out string fieldInvalidMessage)
        {
            ...
            switch (userRegistrationField)
            {
                case FieldConstants.UserRegistrationField.EmailAddress:
                    ...
                    fieldInvalidMessage = (fieldInvalidMessage=="" && _emailExistsDel(fieldValue) ? $"The email address already exists.Please try again{Environment.NewLine}" : fieldInvalidMessage);
                    break;
                ...
                case FieldConstants.UserRegistrationField.PasswordCompare:
                    ...
                    fieldInvalidMessage = (fieldInvalidMessage == "" && !_comparisonFieldsValidDel(fieldValue, FieldArray[(int)FieldConstants.UserRegistrationField.Password])) ? $"Your entry did not match the password{Environment.NewLine}" : fieldInvalidMessage;
                    break;
                case FieldConstants.UserRegistrationField.DateOfBirth:
                    fieldInvalidMessage = (!_requiredValidDel(fieldValue)) ? $"You must enter a value for field:{Enum.GetName(typeof(FieldConstants.UserRegistrationField), userRegistrationField)}{Environment.NewLine}" : fieldInvalidMessage;
                    fieldInvalidMessage = (fieldInvalidMessage == "" && !_dateValidDel(fieldValue, out DateTime validDateTime)) ? $"You did not enter a valid date. Please try again{Environment.NewLine}" : fieldInvalidMessage;
                    break;
                case FieldConstants.UserRegistrationField.PhoneNumber:
                    fieldInvalidMessage = (!_requiredValidDel(fieldValue)) ? $"You must enter a value for field:{Enum.GetName(typeof(FieldConstants.UserRegistrationField), userRegistrationField)}{Environment.NewLine}" : fieldInvalidMessage;
                    fieldInvalidMessage = (fieldInvalidMessage == "" && !_patternMatchValidDel(fieldValue, CommonRegularExpressionValidationPattern.Uk_PhoneNumber_RegEx_Pattern)) ? $"You have not entered a valid phone number{Environment.NewLine}" : fieldInvalidMessage;
                    break;
                ...
                case FieldConstants.UserRegistrationField.PostCode:
                    fieldInvalidMessage = (!_requiredValidDel(fieldValue)) ? $"You must enter a value for field:{Enum.GetName(typeof(FieldConstants.UserRegistrationField), userRegistrationField)}{Environment.NewLine}" : fieldInvalidMessage;
                    fieldInvalidMessage= (fieldInvalidMessage =="" && !_patternMatchValidDel(fieldValue,CommonRegularExpressionValidationPattern.Uk_Post_Code_RegEx_Pattern)) ? $"You did not enter a valid post code.Please try again{Environment.NewLine}":fieldInvalidMessage; break;
                    break;
                default:
